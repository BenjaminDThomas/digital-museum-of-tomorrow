  ...
  async function appendRelatedObjects(container, params) {
    try {
      ...
      records.forEach(record => {
        ...
        `;
        mini.addEventListener('click', () => openArtefactModal(record, record.systemNumber || ''));
        mini.addEventListener('keydown', event => {
          if (event.key === 'Enter' || event.key === ' ') {
            event.preventDefault();
            openArtefactModal(record, record.systemNumber || '');
          }
        });
        ...
        for (const line of lines) {
          try {
            ...
            if (data.message && data.message.content) {
              ...
              assistantMessage.querySelector('.message__text').innerHTML = escHtml(fullResponse).replace(/\n/g, '<br>');
              chatMessages.scrollTop = chatMessages.scrollHeight;
            }
          } catch (_) {
